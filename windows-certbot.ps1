# windows-certbot.ps1
# Windows 환경에서 포트 매핑을 사용하여 Let's Encrypt 인증서 발급
...
try {
    ...
    # 포트 80 바인딩으로 실행 (Windows에서는 --net=host가 제대로 작동하지 않을 수 있음)
    $cmd = "docker run --rm -p 80:80 -v `"$PWD/certbot/conf:/etc/letsencrypt`" certbot/certbot certonly --standalone --preferred-challenges http -d $domain --email $email --agree-tos --no-eff-email --force-renewal"
