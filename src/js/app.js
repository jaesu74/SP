    ...
    if (data && data.length > 0) {
      ...
      try {
        ...
        if (recentSanctions && recentSanctions.length > 0) {
          ...
          } else if (window.EventManager && typeof window.EventManager.displayResults === 'function') {
            window.EventManager.displayResults(recentSanctions);
          } else {
            console.error('결과 표시 함수를 찾을 수 없습니다');
          }
        } else {
          ...
          if (window.SearchComponent && typeof window.SearchComponent.displayResults === 'function') {
            ...
            window.EventManager.displayResults(data.slice(0, 10));
          } else {
            console.error('결과 표시 함수를 찾을 수 없습니다');
          }
        }
      ...
      if (window.SearchComponent && typeof window.SearchComponent.displayResults === 'function') {
        ...
        window.EventManager.displayResults(sampleData);
      } else {
        console.error('결과 표시 함수를 찾을 수 없습니다');
      }
    ...
    if (suggestions.length > 0) {
      ...
      suggestions.forEach(suggestion => {
        ...
        item.addEventListener('click', () => {
          ...
          autocompleteContainer.style.display = 'none';
          // SearchComponent 또는 이벤트 관리자의 검색 함수 호출
          if (window.SearchComponent && typeof window.SearchComponent.performSearch === 'function') {
            window.SearchComponent.performSearch();
          } else if (window.EventManager && typeof window.EventManager.handleSearch === 'function') {
            window.EventManager.handleSearch(suggestion);
          } else if (window.app && typeof window.app.performSearch === 'function') {
            window.app.performSearch();
          } else {
            console.error('검색 함수를 찾을 수 없습니다');
          }
        });
...
window.app = {
  ...
  performSearch: async function(e) {
    ...
    // SearchComponent 또는 EventManager의 performSearch 함수를 호출
    if (window.SearchComponent && typeof window.SearchComponent.performSearch === 'function') {
      await window.SearchComponent.performSearch();
    } else if (window.EventManager && typeof window.EventManager.handleSearch === 'function') {
      const searchInput = document.getElementById('search-input');
      const query = searchInput ? searchInput.value.trim() : '';
      await window.EventManager.handleSearch(query);
    } else {
      console.error('검색 함수를 찾을 수 없습니다');
    }
  },
...
function showAlert(message, type = 'info', options = {}) {
  // 기존 AlertManager가 있으면 사용
  if (window.AlertManager && typeof window.AlertManager.showAlert === 'function') {
    window.AlertManager.showAlert(message, type, options);
    return;
  }

  // 기본 알림 처리
  const container = document.querySelector(options.target || '.alert-container') || document.createElement('div');

  if (!container.classList.contains('alert-container')) {
  ...
  alert.innerHTML = `
        ...
        ${options.isStatic ? '' : '<button class="alert-close">&times;</button>'}
    `;
