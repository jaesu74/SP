...
def normalize_data(data, source):
    ...
    for item in data:
        # 공통 필드
        normalized_item = {
            "id": f"{source.upper()}_{item.get('id', '')}",
            "name": get_name(item),
            "type": get_entity_type(item, source),
            "country": get_country(item, source),
            "programs": [f"{source.upper()}_SANCTIONS"],
        }
        ...
        details = {
            ...
            "birthDate": get_birth_date(item, source),
            "description": get_description(item, source),
            "sanctionDate": get_sanction_date(item, source),
        }
        
        # null 값 제거
        details = {k: v for k, v in details.items() if v is not None}
        normalized_item["details"] = details
...
def get_name(item):
    ...
    return first or last or "Unknown Name"


def get_entity_type(item, source):
    """항목의 유형을 반환합니다."""
    if source == 'un':
        return 'Individual' if 'first_name' in item else 'Entity'
    elif source == 'eu':
        return item.get('entity_type', 'Unknown')
    elif source == 'us':
        return item.get('sdnType', 'Unknown')
...
def get_identifications(item, source):
    """항목의 식별 문서 목록을 반환합니다."""
    if source == 'un':
        return [{"type": doc.split(':')[0], "number": doc.split(':')[1]} if ':' in doc else {"type": "Other", "number": doc} for doc in item.get('documents', [])]
    elif source == 'eu':
        return [{"type": doc.get('type', 'Other'), "number": doc.get('number', '')} for doc in item.get('documents', [])]
    elif source == 'us':
        return [{"type": id_item.get('idType', 'Other'), "number": id_item.get('idNumber', '')} for id_item in item.get('idList', [])]
...
def get_birth_date(item, source):
    """항목의 생년월일을 반환합니다."""
    if source == 'un':
        return item.get('birth_date', None)
    elif source == 'eu':
        return item.get('date_of_birth', None)
    elif source == 'us':
        return item.get('dateOfBirth', None)
...
def get_description(item, source):
    """항목의 설명을 반환합니다."""
    if source == 'un':
        return item.get('comments1', '')
    elif source == 'eu':
        return item.get('remarks', '')
    elif source == 'us':
        return item.get('remarks', '')
...
def get_sanction_date(item, source):
    """항목의 제재 지정일을 반환합니다."""
    if source == 'un':
        return item.get('listed_on', None)
    elif source == 'eu':
        return item.get('listing_date', None)
    elif source == 'us':
        return item.get('listedDate', None)
...
def main():
    ...
    parser.add_argument('-s', '--sources', 
                        help='처리할 데이터 소스 (쉼표로 구분, 기본값: 모든 소스)',
                        default=None)
